...
public class Solution {
    ...
    private static Set<Integer> getRadix(String number) {
        ...
        for (int i = 2; i <= 36; i++) {
            ...
            //проверяем каждую систему на то, подходит ли число
            String sys = from10toX(num,i);

            //если подходит
            if(isPolindromic(sys)) {
                result.add(i);
            }
        }
    ...
    private static String from10toX(int number,int x) {
        ...
        while ( value >= x ) {
            ...
            if (q > 9) {
                result = String.valueOf(alphabet.get(q)) + result;
            }
            else {
                result = String.valueOf(q) + result;
            }
        ...
        if (value > 9) {
            result = String.valueOf(alphabet.get(value)) + result;
        }
        else {
